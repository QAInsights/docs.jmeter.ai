...
//   {/* CUSTOM-FOOTER:END */}
//
// Markers use MDX comment syntax ({/* ... */}) because HTML comments
// (<!-- ... -->) are rejected by the @mdx-js/rolldown parser. The
// extractor accepts both styles for backward compatibility with files
 ...
 */
// Build a regex that matches either the current MDX-comment marker style
// (`{/* NAME */}`) or the legacy HTML-comment style (`<!-- NAME -->`), so
// files written before the MDX-comment migration are still recognized.
function markerRegex(markerName) {
  // MDX comment: {/* NAME */}
  // HTML comment: <!-- NAME -->
  const mdxStyle = `\\{/\\*\\s*${markerName}\\s*\\*/\\}`;
  const htmlStyle = `<!--\\s*${markerName}\\s*-->`;
  return new RegExp(`(?:${mdxStyle}|${htmlStyle})`);
}

function markerIndexOf(rest, markerName) {
  const re = markerRegex(markerName);
  const m = rest.match(re);
  return m ? m.index : -1;
}

function extractMarkerContent(rest, startName, endName) {
  const startRe = markerRegex(startName);
  const endRe = markerRegex(endName);
  const startMatch = rest.match(startRe);
  if (!startMatch) return null;
  const afterStart = rest.slice(startMatch.index + startMatch[0].length);
  const endMatch = afterStart.match(endRe);
  if (!endMatch) return null;
  return afterStart.slice(0, endMatch.index).replace(/^\r?\n/, '').replace(/\r?\n$/, '').trim();
}
...
function extractCustomSections(rest) {
  ...
  // Try marker-based extraction first (supports both MDX and HTML comment styles).
  const introContent = extractMarkerContent(rest, 'CUSTOM-INTRO:START', 'CUSTOM-INTRO:END');
  const footerContent = extractMarkerContent(rest, 'CUSTOM-FOOTER:START', 'CUSTOM-FOOTER:END');

  if (introContent !== null || footerContent !== null) {
    ...
    const markerPositions = [
      markerIndexOf(rest, 'CUSTOM-INTRO:START'),
      markerIndexOf(rest, 'SYNCED-BODY:START'),
      markerIndexOf(rest, 'CUSTOM-FOOTER:START'),
    ].filter(p => p !== -1);
    ...
    if (introContent !== null) result.customIntro = introContent;
    if (footerContent !== null) result.customFooter = footerContent;
    return result;
  }
