...
description: "Get started with Apache JMeter for performance testing. Build test plans in GUI mode, debug with View Results Tree, and run tests in non-GUI mode."
difficulty: "beginner"
guideType: "tutorial"
...
<a id="test_plan_building"></a>

#### 1.0.1 Test plan building

To do that, you will [run JMeter in GUI Mode.](/getting-started/get-started/#running)

Then you can either choose to record the application from a browser, or native application.
You can use for that the menu **File → Templates... → Recording**

Note you can also manually build your plan. Ensure you read this [documentation](/user-manual/test-plan/) to understand major concepts.

You will also debug it using one of these options:

- **Run → Start no pauses**
- **Run → Start**
- **Validate** on [Thread Group](/user-manual/component-reference/#Thread_Group)

and [View Results Tree](/user-manual/component-reference/#View_Results_Tree) renderers or Testers (CSS/JQUERY, JSON, Regexp, XPath).

Ensure you follow [best-practices](/user-manual/best-practices/) when building your Test Plan.

<a id="load_test_running"></a>
...
Using CLI mode, you can generate a CSV (or XML) file containing results and have JMeter [generate an HTML report](/user-manual/generating-dashboard/) at end of Load Test.
JMeter will by default provide a summary of load test while it's running. 

You can also have [real-time results](/user-manual/realtime-results/) during your test using [Backend Listener](/user-manual/component-reference/#Backend_Listener).

<a id="load_test_analysis"></a>
...
Next, start JMeter and go through the [Building a Test Plan](/user-manual/build-test-plan/) section
...
- [Advanced Web Test Plan](/user-manual/build-adv-web-test-plan/)
- [JDBC](/user-manual/build-db-test-plan/)
- [FTP](/user-manual/build-ftp-test-plan/)
- [JMS Point-to-Point](/user-manual/build-jms-point-to-point-test-plan/)
- [JMS Topic](/user-manual/build-jms-topic-test-plan/)
- [LDAP](/user-manual/build-ldap-test-plan/)
- [LDAP Extended](/user-manual/build-ldapext-test-plan/)
- [WebServices (SOAP)](/user-manual/build-ws-test-plan/)
- [Building Test Plan programmatically](/user-manual/build-programmatic-test-plan/)
...
<strong>The JMeter HTTP samplers are configured to accept all certificates,
whether trusted or not, regardless of validity periods, etc.</strong>
...
There is also the [SSL Manager](/user-manual/component-reference/#SSL_Manager), for greater control of certificates.
...
**File → Templates…** or Templates icon:
![Templates icon item](/images/screenshots/template_menu.png)
_Templates icon item_

A popup appears, you can then choose a template among the list:
![Templates popup](/images/screenshots/template_wizard.png)
_Templates popup_
...
![Parameters window](/images/screenshots/templates/template_parameters.png)
_Parameters window_
...
:::note
You can create your own templates following documentation [here](/../creating-templates/)
:::
...
Proxy Settings can also be defined in a Test Plan, using either the [HTTP Request Defaults](/user-manual/component-reference/#HTTP_Request_Defaults)
configuration or the [HTTP Request](/user-manual/component-reference/#HTTP_Request) sampler elements.

:::note
JMeter also has its own in-built Proxy Server, the [HTTP(S) Test Script Recorder](/user-manual/component-reference/#HTTP_S__Test_Script_Recorder).
...
For [distributed testing](/user-manual/remote-test/), run JMeter in server mode on the remote node(s), and then control the server(s) from the GUI.
...
**`-q[propertyfile]`**
: additional JMeter property file
...
| Log Levels in Configuration or Command Line Arguments | Old Log Levels:              - `DEBUG` - `INFO` - `WARN` - `ERROR` - `FATAL_ERROR` - `NONE` | Mapping to New Levels through SLF4J/Log4j2:              - `DEBUG` - `INFO` - `WARN` - `ERROR` - `ERROR` - `OFF`              :::note Since `FATAL_ERROR` is not supported by SLF4J API,               it is treated as `ERROR` instead for existing code not to break.               There is also the `FATAL` log level option. :::              :::note `TRACE` level, which is less specific than `DEBUG`, is supported additionally since 3.2.               Look up SLF4J or Apache Log4J 2 documentations for details. ::: |
...
JMeter does not generally use pop-up dialog boxes for errors, as these would interfere with
    ...
    reference is just used as is. See [Functions and Variables for more information](/user-manual/functions/).
:::
    ...
    If JMeter detects an error during a test, a message will be written to the log file.
        ...
        ![Error/fatal counter](/images/screenshots/log_errors_counter.png)
_Error/fatal counter_
...
| xml.parser | No | You can specify an implementation as your XML parser. |
...
Build your first test plan with the [Building a Test Plan](/user-manual/build-test-plan/) guide.
</Fragment>
<Fragment slot="related">
- [Best Practices](/user-manual/best-practices/)  -  essential guidelines before running load tests
- [Boss](/user-manual/boss/)  -  how to scope a performance testing effort
</Fragment>
<Fragment slot="troubleshooting">
- If JMeter fails to start, verify you have Java 17+ installed (`java -version`)
- Use View Results Tree in Validate mode to debug before running full tests
</Fragment>
</RelatedContent>
{/* CUSTOM-FOOTER:END */}
