...
description: "JMeter glossary — definitions of key performance testing terms including elapsed time, latency, connect time, median, throughput, and standard deviation."
---

## 23. Glossary

<a id="Elapsed"></a>. JMeter measures the elapsed time from just before sending the request to
...
<a id="Latency"></a>. JMeter measures the latency from just before sending the request to
...
<a id="Connect"></a>. JMeter measures the time it took to establish the connection, including SSL handshake. Note that connect time is not automatically subtracted from [latency](#Latency).
...
<a id="Median"></a> is a number which divides the samples into two equal halves.
...
<a id="Percentile"></a> is the value below which 90% of the samples fall.
...
<a id="StandardDeviation"></a> is a measure of the variability
...
<a id="ThreadName"></a> as it appears in Listeners and logfiles
...
<a id="Throughput"></a> is calculated as requests/unit of time.
