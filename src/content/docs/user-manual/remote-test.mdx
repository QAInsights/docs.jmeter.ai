...
description: "Configure JMeter distributed testing to scale load across multiple remote engines. Control worker nodes from a single client and aggregate results centrally."
difficulty: "advanced"
guideType: "reference"
...
If the test uses any data files, <strong>note that these are not sent across by the client so
make sure that these are available in the appropriate directory on each server</strong>.
...
![Figure 1 - Run Menu](/images/screenshots/remote/run-menu00.png)
_Figure 1 - Run Menu_
